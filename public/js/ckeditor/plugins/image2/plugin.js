...
( function() {
	...
	var template = '<img class="" alt="" />',
		templateBlock = new CKEDITOR.template(
			'<figure class="{captionedClass} image-zoom">' +
				template +
	...
	function widgetDef( editor ) {
		...
		function inflate() {
			...
			// According to the new state.
				...
				if (this.element.is('img')) {
					this.element.getParent().addClass('image-zoom');

					if (this.widget.data.cantZoom) {
						this.element.getParent().addClass('cantZoom');
					}

					if (!this.widget.data.cantZoom) {
						this.element.getParent().removeClass('cantZoom');
					}
				}

				if (this.element.is('p')) {
					this.element.addClass('image-zoom');
				}
		...
		return {
			...
			parts: {
				image: 'img',
				caption: 'figcaption',
				zoom: 'figure.cantZoom, img.cantZoom, p.cantZoom'
	...
	CKEDITOR.plugins.image2 = {
		stateShifter: function( editor ) {
			...
			var stateActions = {
				...
				cantZoom: function ( shift, oldValue, newValue ) {
					shift.element.removeClass('cantZoom');

					if (newValue) {
						shift.element.addClass('cantZoom');
					}

					if (!newValue && !shift.element.is('img')) {
						let img = shift.element.findOne('img');
						img.removeClass('cantZoom');
					}
				},
