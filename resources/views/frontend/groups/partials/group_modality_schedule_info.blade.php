@component('admin.layouts.components.card_clean')
    ...
    {{ html()->form('POST', route('web.frontend.football_matches.store'))->open() }}
    ...
    <div class="table-responsive">        
        <table class="table table-striped footballer-data">
            ...
            <tr>             
             ...
             @if(!empty($schedule->footballMatches))  
                @foreach ($schedule->footballMatches as $match)
                   <tr>
                       <td class="shrink"><strong> Partida: </strong></td>
                       <td>
                           {{ \Carbon\Carbon::parse($match->match_datetime)->translatedFormat('l d/m/Y \à\s H:i:s') }}
                       </td>
                       <td class="shrink"><strong>{{ modelAttribute($type_group, 'closing_time') }}</strong></td>
                       <td>
                          {{ $schedule->closing_time }}
